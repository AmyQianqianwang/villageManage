...
#userTable tr td{overflow: hidden;white-space:nowrap; text-overflow: ellipsis;} 
  ...
  <body>
	...
	<div class="mainTable">
		...
		<div class="forSN">
			<div class="inforSN">
				<form method="post" action="${basePath}record_list.action">
				...
				<c:if test="${!empty proList}">
					<c:forEach items="${proList}" var="r" varStatus="s">
						<tr class="dataTable">
							...
							<td>
							<c:if test="${r.proStatus eq 14}">
							<img src="${basePath}images/icon_check.png" alt="完成且通过" title="完成且通过"/>
							</c:if>
							<c:if test="${r.proStatus ne 14}">
							<img src="${basePath}images/icon_error.png" alt="完成未通过" title="完成未通过"/>
							</c:if>
							</td> 
